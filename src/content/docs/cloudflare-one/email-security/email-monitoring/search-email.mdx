...
5. **Attachments**: If an email has an attachment, the Cloudflare dashboard will display the filename, and the disposition assigned. You can open attachments in [Browser Isolation](/cloudflare-one/policies/browser-isolation/). Only PDF files are currently supported.
...
:::note
If you purchased Gateway and Remote Browser Isolation, use these instead of Email Security's Email Link Isolation as more customizable. 

Email Security Email Link Isolation cannot access Remote Browser Isolation's **Clipboard**, **Device Interaction**, and **File Access** policy settings on the Cloudflare dashboard.
:::
